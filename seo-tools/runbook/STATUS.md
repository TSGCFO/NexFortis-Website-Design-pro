...
### PHASE B WAVE 1 — architecture IMPLEMENTED + page #1 ✅ COMPLETE
...
**Done (2/15):** #1 `local-seo/toronto`, #2 `seo/mississauga` — both live on the PR-108 preview, all gates green, fixtures committed. Page #2 needed ZERO architecture changes (the page-#1 plumbing + prerender dedupe fix held).
**Next = page #3: `seo services — Markham`** (vol 500, KD 0). Run the same 8-step run book per the build order in `geo/03-build-waves.md` (Wave 1 = 15 pages). The pipeline + architecture are proven; repeat per page: keyword cluster (**city-qualified only** in `keyword-ownership.json`) → KI brief → KI outline (**review for fabrication**) → Claude draft (grounded in `_facts.md`) → gates → integrate (typed `_geoContent.tsx` entry + flip `geo-links.ts` `published:true` + `node scripts/gen-geo-routes.mjs`) → QA on the Render preview + regenerate the page fixture & sitemap snapshot. Wave list + all 4 waves: `geo/03-build-waves.md`; locked matrix: `geo/02-demand-matrix.md`.
