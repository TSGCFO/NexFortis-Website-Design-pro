...
/**
 * Deduplicate SEO-relevant tags inside <head>.
 *
 * Pre-rendered HTML contains BOTH the static SEO tags from index.html (the SPA
 * shell fallback) AND the per-page tags injected at runtime by
 * react-helmet-async. Crawlers must see exactly one canonical/title/desc/OG per
 * page, so we dedupe:
 *
 *   - <title>: keep the FIRST occurrence (React 19 / helmet hoists its title
 *     before the shell's pre-existing <title> tag).
 *   - Everything else (meta name/property/itemprop/charset/http-equiv, link
 *     rel=canonical/alternate): keep the LAST occurrence (helmet appends after
 *     the shell's static tags, so the per-page version wins).
 */
