...
function matchMeta(html, name) {
  ...
  for (const m of html.matchAll(tagRe)) {
    const attrs = m[1];
    const nameRe = new RegExp(`\\bname\\s*=\\s*["']${escapeRegex(name)}["']`, "i");
...
function matchProperty(html, property) {
  ...
  for (const m of html.matchAll(tagRe)) {
    const attrs = m[1];
    const propRe = new RegExp(`\\bproperty\\s*=\\s*["']${escapeRegex(property)}["']`, "i");
...
function matchLink(html, rel) {
  ...
  for (const m of html.matchAll(tagRe)) {
    const attrs = m[1];
    const relRe = new RegExp(`\\brel\\s*=\\s*["']${escapeRegex(rel)}["']`, "i");
...
function isRootDivEmpty(html) {
  // Find any <div ...> whose attributes contain id="root" (in any position).
  const divRe = /<div\b([^>]*)>/gi;
  let openMatch;
  while ((openMatch = divRe.exec(html)) !== null) {
    const attrs = openMatch[1];
    if (!/\bid\s*=\s*["']root["']/i.test(attrs)) continue;
