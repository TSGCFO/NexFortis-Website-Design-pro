const SEO_DEDUPE_KEYS = new Set([
  ...
  "twitter:image",
  "og:image:type",
  // geo.* are per-page on geo [service]+[city] pages (city, not the HQ). Helmet
  // emits the page's value LAST, so keep-last dedupe yields the correct city —
  // and collapses the harmless 2x HQ default on every national page to 1x.
  "geo.region",
  "geo.placename",
]);
